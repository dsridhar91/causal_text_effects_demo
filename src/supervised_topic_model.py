...
class SupervisedTopicModel(nn.Module):
    def __init__(self, num_topics, vocab_size, num_documents, t_hidden_size=300, theta_act='relu', enc_drop=0., outcome_linear_map=True):
    ...
    def get_beta(self):
        beta = self.alphas #F.softmax(self.alphas, dim=0).transpose(1, 0) ## softmax over vocab dimension
        return beta
